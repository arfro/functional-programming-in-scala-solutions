...
class ListSpec extends FlatSpec with Matchers {
  ...
  it should "return Nil for one element list" in {
    CustomList.removeTail(Cons(1, Nil)) shouldBe Nil
  }

  it should "return 2 for Cons 1, 2" in {
  ...
  it should "drop first element" in {
  ...
  it should "drop all if drop number larger than list length" in {
    CustomList.drop(Cons(1, Cons(2, Cons(3, Cons(4, Nil)))), 9) shouldBe Nil
  }

  it should "not drop anything is drop number is zero" in {
  ...
  it should "return 0 for size 0" in {
    // if the function dropWhile wasn't curried then we would have to specify x type
  ...
  it should "returns empty list for emtpty list" in {
    // if the function dropWhile wasn't curried then we would have to specify x type
  ...
  it should "returns 1 2 3 4 for 1 2 & 3 4" in {
    // if the function dropWhile wasn't curried then we would have to specify x type
  ...
  it should "returns 1 2 3 4 doubled" in {
    // if the function dropWhile wasn't curried then we would have to specify x type
